...
import { MoreVertical, ChevronLeft, ChevronRight, X, Users } from 'lucide-react'
    ...
    <>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7}>
                    <EmptyState
                      ...
                      action={(currentRole === 'admin' || currentRole === 'hr') ? {
                ...
                users.map(u => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <StatusBadge isActive={u.isActive} offboarding={u.offboardingStatus === 'in_progress'} />
                    </td>
                    <td className="px-6 py-4">
      ...
      {selected.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 text-white rounded-xl px-6 py-3 flex items-center gap-4 shadow-xl z-50">
          ...
          <button onClick={onBulkDeactivate} className="text-sm bg-red-500 hover:bg-red-600 px-3 py-1.5 rounded-md">Désactiver</button>
          <button onClick={onBulkExport} className="text-sm bg-blue-500 hover:bg-blue-600 px-3 py-1.5 rounded-md">Exporter CSV</button>
          <button onClick={onClearSelection} className="text-slate-400 hover:text-white ml-2"><X size={16} /></button>
        </div>
      )}
    </>
  )
