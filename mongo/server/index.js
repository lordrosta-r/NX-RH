...
v1Router.use('/branding',         apiLimiter, authenticated, require('./routes/branding'))
...
async function start() {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    // RELAX_RATE_LIMIT relâche le rate-limit de login. Interdit en VRAIE prod,
    // mais autorisé pour une stack e2e JETABLE qui se déclare explicitement via
    // E2E_STACK=true (double opt-in : impossible à activer par accident en prod).
    if (process.env.RELAX_RATE_LIMIT === 'true' && process.env.E2E_STACK !== 'true') {
      fatal.push('RELAX_RATE_LIMIT=true affaiblit la protection anti-bruteforce du login — interdit en production (stack e2e jetable : ajouter E2E_STACK=true en connaissance de cause)')
    }
