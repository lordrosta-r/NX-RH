...
export const handlers = [
  ...
  http.get("http://localhost:5050/api/users/:id", ({ params }) =>
    HttpResponse.json({
      data: { ...mockUser, _id: params.id as string, id: params.id as string },
    }),
  ),
  http.patch(
  ...
  http.get(
    "http://localhost:5050/api/users/:id/gdpr-export",
    () =>
  ...
  http.post("http://localhost:5050/api/campaigns", async ({ request }) => {
    const body = (await request.json()) as Record<string, unknown>;
    return HttpResponse.json(
      { data: { ...mockCampaign, ...body, _id: "camp-new" } },
      { status: 201 },
    );
  }),
  http.patch(
    "http://localhost:5050/api/campaigns/:id",
    async ({ request, params }) => {
      const body = (await request.json()) as Record<string, unknown>;
      return HttpResponse.json({
        data: { ...mockCampaign, _id: params.id as string, ...body },
      });
    },
  ),
  ...
  http.get(
    "http://localhost:5050/api/evaluations/:id/pdf",
  ...
  http.get(
    "http://localhost:5050/api/analytics/export/csv",
  ...
  http.get("http://localhost:5050/api/events", () =>
    HttpResponse.json({
      data: [
        { _id: "evt-1", title: "Réunion", date: "2025-06-01", type: "meeting" },
      ],
      total: 1,
    }),
  ),
  ...
  http.get("http://localhost:5050/api/offboarding/:id", ({ params }) =>
    HttpResponse.json({
      ...
      lastDay: "2025-07-01",
      status: "pending",
      checklist: [{ label: "Badge retourné", done: false }],
    }),
  ),
  ...
  http.get(
    "http://localhost:5050/api/admin/audit/export",
    () =>
      new HttpResponse(
        new Blob(["date,action\n2025-01-01,activate"], { type: "text/csv" }),
      ),
  ),
