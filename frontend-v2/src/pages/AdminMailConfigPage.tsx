...
export default function AdminMailConfigPage() {
  ...
  const testMutation = useMutation({
    mutationFn: (email: string) => client.post('/api/admin/mail/test', { to: email }),
  })
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      <PageHeader title="Configuration Email" subtitle="Paramètres SMTP et envoi d'emails" />
      ...
      {isLoading ? (
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="secure" className="text-sm text-gray-700">Connexion sécurisée (TLS)</label>
          </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 space-y-3">
        ...
        {testMutation.isSuccess && <p className="text-sm text-green-600">✅ Email envoyé avec succès</p>}
        {testMutation.isError && <p className="text-sm text-red-600">❌ Échec de l'envoi</p>}
      </div>
    </div>
  )
}
