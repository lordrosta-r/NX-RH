...
function matchesFilters(
  ...
  filters: { campaignId?: string | null; status?: string | null; q?: string | null; year?: string | null }
) {
  if (filters.campaignId && evaluation.campaignId !== filters.campaignId) return false
  if (filters.status && evaluation.status !== filters.status) return false
  if (filters.year && evaluation.createdAt?.slice(0, 4) !== filters.year) return false
  ...
  it('affiche la liste avec les colonnes attendues et filtre par campagne/statut/recherche', async () => {
    server.use(
      http.get('http://localhost:5050/api/evaluations', ({ request }) => {
        ...
        const data = adminEvaluations.filter(e => matchesFilters(e, { campaignId, status, q }))
    ...
    await waitFor(() => expect(within(table).getAllByRole('link', { name: 'Bruno Petit' })).toHaveLength(1))
  })
  ...
  it('affiche les évaluations employé en cards et pas en tableau', async () => {
    server.use(
      http.get('http://localhost:5050/api/evaluations/me', ({ request }) => {
        ...
        const campaignId = url.searchParams.get('campaignId')
        const status = url.searchParams.get('status')
        const data = employeeEvaluations.filter(e => matchesFilters(e, { campaignId, status }))
        return paginated(data)
      })
    )
    ...
    await waitFor(() => expect(screen.getAllByText('Formulaire annuel').length).toBeGreaterThan(0))
    expect(screen.queryByRole('table')).not.toBeInTheDocument()
    expect(await screen.findByRole('link', { name: 'Remplir' })).toBeInTheDocument()
  })

  it('affiche les bulk checkboxes pour admin et navigue vers le détail', async () => {
    server.use(
      http.get('http://localhost:5050/api/evaluations', () => paginated(adminEvaluations))
    )

    renderWithProviders(
      <Routes>
        <Route path="/evaluations" element={<EvaluationsPage />} />
        <Route path="/evaluations/:id" element={<div>Détail évaluations</div>} />
      </Routes>,
      { initialEntries: ['/evaluations'], user: admin }
    )

    const table = await screen.findByRole('table')
    await waitFor(() => expect(within(table).getAllByRole('link', { name: 'Alice Martin' }).length).toBeGreaterThan(0))
    await waitFor(() => expect(within(table).getAllByRole('checkbox')).toHaveLength(4))
    ...
    await user.click(within(table).getAllByRole('link', { name: 'Alice Martin' })[0])
...
describe('EvaluationDetailPage', () => {
  it('mode A: affiche le formulaire, autosave et confirmation de soumission', async () => {
    ...
    server.use(
      http.get('http://localhost:5050/api/evaluations/:id', ({ params }) =>
        HttpResponse.json(makeEvaluation({
      ...
      http.patch('http://localhost:5050/api/evaluations/:id', async ({ request }) => {
        patchCalls.push({ body: await request.json() as Record<string, unknown> })
    ...
    expect(screen.getByText('Qu’avez-vous accompli cette année ?')).toBeInTheDocument()

    await user.type(screen.getByRole('textbox'), 'Bilan complet')
    await waitFor(() => expect(patchCalls).toHaveLength(1), { timeout: 3000 })
    expect(patchCalls[0].body.answers).toMatchObject({ 'q-1': 'Bilan complet' })

    await user.click(screen.getByRole('button', { name: 'Suivant →' }))
    expect(screen.getByText('Quels sont vos objectifs ?')).toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: /soumettre l'évaluation/i }))
  ...
  it('mode B: affiche les réponses et les champs de révision', async () => {
    ...
    expect(await screen.findByText('Révision de l\'évaluation')).toBeInTheDocument()
    expect(screen.getByText('Très bon travail')).toBeInTheDocument()
    expect(screen.getByRole('spinbutton')).toBeInTheDocument()
    expect(screen.getAllByRole('textbox')).toHaveLength(2)
    expect(screen.getByRole('button', { name: 'Enregistrer la révision' })).toBeInTheDocument()
  })
  ...
  it('mode C: affiche le stepper et le bouton contextuel de signature', async () => {
    ...
    expect(await screen.findByText("Compte-rendu d'entretien")).toBeInTheDocument()
  ...
  it('mode C: permet de cocher le désaccord côté évalué', async () => {
    ...
    expect(await screen.findByText('Votre prise de connaissance')).toBeInTheDocument()
    expect(screen.getByRole('checkbox', { name: /désaccord/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Signer et valider la prise de connaissance' })).toBeInTheDocument()
  })
  ...
  it('mode D: affiche le compte-rendu complet et le PDF', async () => {
    ...
    expect(await screen.findByText(/Compte-rendu — Alice Martin/)).toBeInTheDocument()
    ...
    expect(window.open).toHaveBeenCalledWith('/api/evaluations/eval-1/pdf', '_blank')
  })
})
...
describe('EvaluationHistoryPage', () => {
  it('affiche les évaluations validées en cards et filtre par année', async () => {
    ...
    expect(await screen.findByText("Mon historique d'entretiens")).toBeInTheDocument()
    ...
    await waitFor(() => {
      expect(screen.getAllByRole('link', { name: /Voir le compte-rendu/i })).toHaveLength(1)
    })
    expect(screen.getAllByText('Évaluation mi-année 2024').length).toBeGreaterThan(0)
  })
})
