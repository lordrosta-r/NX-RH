...
export default function SearchOverlay({ open, onClose }: SearchOverlayProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect -- reset à la fermeture, volontaire
      setQ("");
    }
  }, [open]);
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-start justify-center pt-16 p-4 bg-black/50"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100">
          ...
          <input
            ref={inputRef}
            aria-label="Rechercher utilisateurs, campagnes, formulaires"
          ...
          {q.trim().length < 2 ? (
            <div className="p-6 text-center text-slate-600 text-sm">
              ...
              <p className="mt-2 text-xs text-slate-300">
                Raccourci :{" "}
                <kbd className="px-1.5 py-0.5 bg-slate-100 rounded text-slate-500 font-mono text-xs">
                  Ctrl
                </kbd>{" "}
                +{" "}
                <kbd className="px-1.5 py-0.5 bg-slate-100 rounded text-slate-500 font-mono text-xs">
