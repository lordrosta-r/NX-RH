...
export const offboardingApi = {
  getOffboardings: (params?: OffboardingFilters) =>
    client.get<PaginatedResponse<OffboardingRecord>>("/api/offboarding", {
      params,
    }),
  ...
  toggleChecklistItem: (id: string, index: number) =>
    client.patch<OffboardingRecord>(
      `/api/offboarding/${id}/checklist/${index}`,
    ),

  updateNotes: (id: string, notes: string) =>
    client.patch(`/api/offboarding/${id}`, { notes }),

  changeStatus: (id: string, status: string) =>
    client.patch(`/api/offboarding/${id}`, { status }),

  deleteOffboarding: (id: string) => client.delete(`/api/offboarding/${id}`),
};
